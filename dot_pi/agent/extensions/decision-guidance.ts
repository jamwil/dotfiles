...
const GUIDANCE_RULES: GuidanceRule[] = [
    {
        ...
        priority: 10,
        detect: (ctx) => {
            const recentErrors = ctx.recentErrors.filter((e) => e.turnIndex >= ctx.turnCount - 3)
...
3. Consider alternative approaches if the current path keeps failing`,
...

    {
        ...
        priority: 9,
        detect: (ctx) => {
            const recentCalls = ctx.recentToolCalls.filter((c) => c.turnIndex >= ctx.turnCount - 4)
...
- If stuck, consider starting fresh with lessons learned`,
...

    {
        ...
        priority: 6,
        detect: (ctx) => {
            const lastTurns = ctx.recentToolCalls.filter((c) => c.turnIndex >= ctx.turnCount - 3)
...
function buildTrajectoryContext(entries: unknown[], currentTurnCount: number): TrajectoryContext {
...
export default function decisionGuidance(pi: ExtensionAPI): void {
    ...
    pi.on("turn_end", async (_event, ctx) => {
        if (lastInjectedGuidance.length > 0 && ctx.hasUI) {
            ...
            ctx.ui.setStatus("guidance", ctx.ui.theme.fg("warning", `⚡ guidance: ${ruleIds}`))
