...
const CodeSearchParamsSchema = Type.Object({
    ...
    regex: Type.Optional(
        Type.Boolean({
            description:
                "Treat query as regular expression. Prefix with (?s) to match across lines",
        }),
    ),
    ...
    lang: Type.Optional(
        Type.Array(Type.String(), {
            description: "Filter by languages (e.g., ['TypeScript', 'TSX'])",
        }),
    ),
})
...
function formatResultsAsText(results: SearchResult[]): string {
    return results
        .map((r) => {
            const snippetsText = r.snippets
                .map((s) => `Line ${s.lineNumber}:\n${s.code}`)
                .join("\n\n")
...
export default function (pi: ExtensionAPI) {
    pi.registerTool({
        ...
        renderResult(result, { expanded, isPartial }, theme) {
            ...
            container.addChild(
                new Text(
                    theme.fg("success", "✓ ") + theme.fg("muted", `${results.length} repos`),
                    0,
                    0,
                ),
            )
            ...
            for (let i = 0; i < maxResults; i++) {
                ...
                for (let j = 0; j < Math.min(maxSnippets, r.snippets.length); j++) {
                    ...
                    // Line number
                    container.addChild(
                        new Text(theme.fg("dim", `Line ${snippet.lineNumber}:`), 0, 0),
                    )
                ...
                if (!expanded && r.snippets.length > 1) {
                    container.addChild(
                        new Text(
                            theme.fg("dim", `... ${r.snippets.length - 1} more snippets`),
                            0,
                            0,
                        ),
                    )
            ...
            if (!expanded && (hiddenResults > 0 || totalSnippets > maxResults)) {
                ...
                container.addChild(
                    new Text(
                        theme.fg("dim", `\n... ${more}, `) + rawKeyHint("ctrl+o", "to expand"),
                        0,
                        0,
                    ),
                )
