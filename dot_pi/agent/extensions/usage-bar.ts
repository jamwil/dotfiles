...
async function fetchClaudeUsage(tokenOverride?: string): Promise<UsageSnapshot> {
    const token = tokenOverride ?? loadClaudeToken()
    if (!token) {
        return {
            provider: "anthropic",
            displayName: "Claude",
...
async function fetchCopilotUsage(
    ...
    const token = tokenOverride ?? loadCopilotRefreshToken()
    if (!token) {
        return { provider: "copilot", displayName: "Copilot", windows: [], error: "No credentials" }
    }
...
async function fetchGeminiUsage(
    ...
    const token = tokenOverride ?? loadGeminiToken()
    if (!token) {
...
async function loadCodexAuth(modelRegistry: any): Promise<CodexAuth | undefined> {
...
async function fetchCodexUsage(
    modelRegistry: any,
    authOverride?: CodexAuth,
): Promise<UsageSnapshot> {
    const auth = authOverride ?? (await loadCodexAuth(modelRegistry))
    if (!auth?.accessToken) {
...
class UsageComponent {
    ...
    private async load() {
        ...
        // Only make network requests for providers that appear to be configured.
        const claudeToken = loadClaudeToken()
        const copilotToken = loadCopilotRefreshToken()
        const geminiToken = loadGeminiToken()
        const codexAuth = await loadCodexAuth(this.modelRegistry)

        // Fetch usage and status in parallel (only for configured providers)
        const [
            claude,
            copilot,
            gemini,
            ...
            claudeToken
                ? timeout(fetchClaudeUsage(claudeToken), 6000, {
                      provider: "anthropic",
                      displayName: "Claude",
                      windows: [],
                      error: "Timeout",
            ...
            copilotToken
                ? timeout(fetchCopilotUsage(this.modelRegistry, copilotToken), 6000, {
                      provider: "copilot",
                      displayName: "Copilot",
                      windows: [],
                      error: "Timeout",
            ...
            geminiToken
                ? timeout(fetchGeminiUsage(this.modelRegistry, geminiToken), 6000, {
                      provider: "gemini",
                      displayName: "Gemini",
                      windows: [],
                      error: "Timeout",
            ...
            codexAuth
                ? timeout(fetchCodexUsage(this.modelRegistry, codexAuth), 6000, {
                      provider: "codex",
                      displayName: "Codex",
                      windows: [],
                      error: "Timeout",
        ...
        // Filter out providers that aren't configured (no credentials)
        const allUsages = [claude, copilot, gemini, codex]
        this.usages = allUsages.filter((u) => u.windows.length > 0 || u.error !== "No credentials")
        this.loading = false
            ...
            for (const u of this.usages) {
                ...
                if (u.error) {
                    ...
                    for (const w of u.windows) {
                        ...
                        const color =
                            remaining <= 10 ? "error" : remaining <= 30 ? "warning" : "success"
