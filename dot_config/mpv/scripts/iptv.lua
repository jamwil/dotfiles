...
-- plsfiltered — список индексов выбранных фильтром элементов плейлиста
  ...
  show = function(self)
    ...
    local count = #self.plsfiltered
    local selected = count > 0 and self.wndstart+self.cursor or 0
    local header
    if string.len(pattern) > 0 then
      header = string.format("Filter: %s    %d / %d matches", pattern, selected, count)
    else
      header = string.format("Channels: %d / %d", selected, count)
    end
    ...
    if count == 0 then
      ...
      if self.wndstart+window-1<count then
        msg = msg.."..."
      end
    end

    local help = "Enter: play  •  Esc: close  •  ↑/↓: navigate  •  type: filter  •  Ctrl+U: clear"
    mp.osd_message("\n"..header.."\n"..msg.."\n"..help, osd_time)
  end,
  ...
  down = function(self)
    if #self.plsfiltered == 0 then return end
...
--[[    mp.add_key_binding('а', 'search1000', typing('а'),"repeatable")
  ...
  keybinder.add("plsclear", clearfilter)
...
function play()
  if not playlister.plsfiltered or #playlister.plsfiltered == 0 then
    return
  end
...
function start()
  ...
  -- Intercept playlists before mpv opens their first entry. Using the
  -- asynchronous start-file event can let FFmpeg begin (and then abort) a
  -- network connection, which produces misleading TLS errors.
  mp.add_hook("on_before_start_file", 50, on_before_start_file)
